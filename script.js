...
//mazda.__proto__ = car; // использование метода __proto__ позволяет подключится к обекту и использовать его свойства и методы
// но этот формат использования прототипа устаревший! 

mazda.saySomething(); // Новая методика исопльзования prototype (объект которому будет назначен prototype
//, прототип объекта, который мы устанавливаем)
...
// Если по ошибке забыли добавить файл в коммит, в то время когда остальные файлы закоммичены:
// 1) git add название файла 2) git commit --amend добавляет к уже закоммиченым файлам недостающие (заменяет старый коммит на новый)
// следовательно, меняется его хеш код!
// git checkout название файла, позволяет откатить изменения на stage этапе, (если он был удалён, или что-то изменили)
...
// git reset --soft хещ код, позволяет вернуться в то состояние, которое было на этапе с тем хеш-кодом не затронув изменения
// есть какие-то изменения
// git revert хеш код не правильного(ошибочного) коммита, таким образом он переносит этот коммит вконец истории, тем самым 
// исправить ошибки, не нарушив (цепочку) историю остальных коммитов 
// git merge куда откуда (напр. feature master) 
